...
- 管理 API
  ...
  - 配额与用量：阈值配置与用量统计，采用两表方案（tts_quota 与 tts_usage）。
...
- ai_tts_voice（共享音色库）
  - 以现有结构为准；业务相关键：
    ...
    - name 等展示字段按现有表使用
  - 权限：仅管理员可新增/编辑/删除，普通用户只读；用于“共享音色”，管理员创建后全体可用（按模型筛选）。
  - 现状：此表已存在（见 `TimbreEntity @TableName("ai_tts_voice")` 与多处 changelog）。

- sys_params（系统参数表）
  - 新增参数建议（仅管理员可写）：
    - param_code: `indextts.base_url` value_type: string 用于管理台合成与克隆适配器
    - param_code: `indextts.api_key` value_type: string 仅后端读取 前端不回显
    - param_code: `indextts.timeout_ms` value_type: number 合成与克隆请求超时毫秒（建议 3000-20000）
  - 可选：`indextts.retry` 根据需要添加（重试次数）
  - 参数配置（前端）：在“系统参数”页面配置（接口前缀 `admin/params`，仅 `sys:role:superAdmin` 可写）
    ...
    - UI 规范：`indextts.base_url` 明文显示；`indextts.api_key` 输入框遮挡，保存后不回显完整值，仅显示“已配置”；`indextts.timeout_ms` 数字输入并做范围校验。
    - 服务读取：`SysParamsService.getValue('indextts.base_url', true)`、`SysParamsService.getValue('indextts.api_key', true)`、`SysParamsService.getValue('indextts.timeout_ms', true)`
...
- tts_quota（用户配额阈值与已用 必要）
  - id bigint pk 自增
  - user_id bigint 唯一索引
  - char_limit bigint call_limit bigint
  - char_used bigint call_used bigint
  - slots int 可用音色位上限 可空
  - updated_at datetime

- tts_usage（每次扣减流水 必要）
  - id bigint pk 自增
  - user_id bigint 索引
  - agent_id bigint 可空 endpoint varchar clone|tts|test
  - cost_chars int cost_calls int duration_ms int
  - (opt) slot_id varchar 便于对账
  - created_at datetime

### 配额与用量（采用方案 B：新增 tts_quota 与 tts_usage）
...
- 引擎配置 `POST /config/agent-models`
 ...
 - 配额与用量（方案 B）
  - `GET /api/tts/quotas` 读取 `tts_quota`；`POST /api/tts/quotas` 管理端设置阈值
  - `POST /api/tts/usage` 写入 `tts_usage`（建议携带 slotId），服务可异步聚合给看板
...
- 前端改动（最小）
  ...
  - 音色位与克隆：新增“音色位管理”（管理员：分配/禁用/设限；用户：查看剩余次数）；从“我的音色位”选择 `slotId` 调用 `POST /ttsVoice/clone`，不展示 raw `voiceId`。
...
- 合成与用量：测试首帧 < 2s、成功率 ≥ 95%；`/tts/quotas` 返回最新计数并触发超限提示。
...
- 管理 API 对前端（统一字段与映射）
  ...
  - 合成测试（新）：`POST /tts/test/speak`
    ...
    - Resp：`audio/wav`
  - 配额与用量（方案 B）：
    - `GET /tts/quotas` → 读取 `tts_quota` 返回 `{ charLimit, callLimit, charUsed, callUsed, slots }`
    - `POST /tts/usage` → 写入 `tts_usage`，用于对账与看板
...
- 替换步骤（最小改造）
  ...
  6 计费联动：克隆按 `files_accepted` 递增 slot.clone_used；合成成功写入 `tts_usage`（可携 slotId），看板聚合自 usage。
